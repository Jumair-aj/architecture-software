...
console.log("📝 Processed Data:", sapTags.length);
      ...
      const processedData = epeTags.map((drawing, i) => {
        ...
        return {
          ...
          "Equipment Type - New": '',
          "Size - Old": merMatch ? merMatch['Size - Old'] ? merMatch['Size - Old'] : merMatch['Size - New'] ? "" : "NOT AVAILABLE" : 'NOT AVAILABLE',
          "Size From SAP": '',
      ...
      merTags.forEach((merRow) => {
        if (!epeTags.some((drawing) => drawing["Tag Number"] === merRow["MER TAG NO"])) {
          ...
          if (merRow.filename.split('-').length > 3 || merRow.filename.length != 14) {
            dngNo = merRow.filename
          }
          else {
            dngNo = merRow.filename.slice(0, 4) + '-' + merRow.filename.slice(4, 5) + '-' + merRow.filename.slice(5, 7) + '-' + merRow.filename.slice(7, 11) + '-' + merRow.filename.slice(11)
          }
          processedData.push({
            ...
            "Final MER Tag No": merRow ? (merRow['SITE CHANGE'] && merRow['SITE CHANGE'].includes("NO")) ? merRow['MER TAG NO'] : merRow['SITE CHANGE'] ? merRow['SITE CHANGE'] : merRow['MER TAG NO'] : "",
            "SAP tag": 'NOT IN SAP',
            "MERRemarks": '',
            "Equipment Description from SAP": "",
            "Equipment Type - New": '',
            "Size - Old": merRow ? merRow['Size - Old'] || merRow['Size - New'] ? "" : "NOT AVAILABLE" : '',
            "Size From SAP": '',
